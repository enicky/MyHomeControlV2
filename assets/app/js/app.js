...
(function(){
    ...
    angular
        .module('app.charts')
        .controller('SensorFlotChartController', SensorFlotChartController);

    SensorFlotChartController.$inject = ['$scope','ChartData','$timeout', '$stateParams'];
    function SensorFlotChartController($scope, ChartData, $timeout, $stateParams){
        var vm = this;
        activate();
            ...
            vm.splineOptions = {
                ...
                tooltipOpts: {
                    content: function (label, x, y) {
                        ...
                        return date.format("{dd}/{MM}/{yyyy}") + ' : ' + y;
                    }
                },
                xaxis: {
        ...
        function activate() {
          ...
          vm.login = function() {
            ...
            if(vm.loginForm.$valid) {
              ...
              $http
                .post('/api/account/login', {username: vm.account.email, password: vm.account.password})
                .then(function(response) {
    ...
    function routesConfig($stateProvider, $locationProvider, $urlRouterProvider, helper){
        ...
        $stateProvider
          .state('app', {
            ...
            .state('app.sensordashboard',{
                url:'/sensordashboard',
                title : 'Sensor Dashboard',
                templateUrl : helper.basepath('sensordashboard.html'),
                resolve: helper.resolveFor('datatables')
            })
            .state('app.sensor',{
                url : '/sensor/:id',
                title : 'Sensor',
                templateUrl : helper.basepath('sensordetail.html'),
                resolve: helper.resolveFor('flot-chart','flot-chart-plugins')
            })
            .state('app.sensorsettings',{
                url : '/settings',
                title : 'SensorSettings',
                templateUrl : helper.basepath('sensorsettings.html'),
                resolve : helper.resolveFor('datatables')
            })
            .state('app.sensorsettingsdetail',{
                url:'/settings/sensor/:id',
...
(function(){
    ...
    angular.module('app.live')
        .controller('SensorSettingsController', SensorSettingsController);
    SensorSettingsController.$inject = ['$resource','$log','$scope'];
    function SensorSettingsController($resource, $log, $scope){
        var vm = this;
        activate();
        function activate(){
            $log.log('Start SettingsController');
            $resource('/api/settings').query().$promise.then(function(sensors){
                $log.log('Got Sensors', sensors);
                vm.sensors = sensors;
            });
        }
    }
...
(function(){
    ...
    LiveController.$inject = ['$resource',  '$log', '$scope'];
    function LiveController($resource, $log, $scope){
        var vm = this;
        activate();
        function activate(){
            $log.log('Send request to latest readigns ????');
            $resource('/api/latestReadings').query().$promise.then(function(readings){
                $log.log('Got readings : ', readings);
                vm.readings = readings;
            });
            $resource('/api/getSwitches').query().$promise.then(function(switches){
...
(function() {
    ...
    function DataTableController($resource, DTOptionsBuilder, DTColumnDefBuilder) {
        ...
        function activate() {
          ...
          $resource('/api/sensors').query().$promise.then(function(sensors) {
             vm.sensors = sensors;
          });
